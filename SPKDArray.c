...
kd_array->size = 0;
kd_array->copy_arr = (SPPoint**) calloc(size, sizeof(*(kd_array->copy_arr)));
...
kd_array->mat = (int**) calloc(spPCADimention, sizeof(*(kd_array->mat)));
if (!kd_array->mat) {
	spKdArrayDestroy(kd_array);
	return NULL;
}
...
for (i = 0; i < spPCADimention; i++) {
	kd_array->mat[i] = (int*) spPointSortByCoor(kd_array->copy_arr, size, i);
...
kdRight->mat = (int**) calloc(spPCADimention, sizeof(*(kdRight->mat)));
if (!kdRight->mat) {
...
kdLeft->copy_arr = (SPPoint*) calloc(kdLeft->size, sizeof(*kdLeft->copy_arr));
if (!kdLeft->copy_arr) {
	//MEM alloc
	spKdArrayDestroy(kdRight);
	free(kdLeft);
...
kdLeft->mat = (int**) calloc(spPCADimention, sizeof(*(kdLeft->mat)));
if (!kdLeft->mat) {
	spKdArrayDestroy(kdRight);
